...
namespace FarsiLibrary.Win
{
    ...
    public class FATabStrip : BaseStyledPanel, ISupportInitialize, IDisposable
    {
        ...
        internal static int PreferredWidth = 200;
        internal static int PreferredHeight = 999;

        #endregion

        #region Constants

        private static int DEF_HEADER_HEIGHT = 19;
        ...
        #region Events
        public event EventHandler TabStripItemDBClicked;
        ...
        /// <summary>
        /// SetVertex a <see cref="FATabStripItem"/> to this control without selecting it.
        /// </summary>
        ...
        /// <summary>
        /// SetVertex a <see cref="FATabStripItem"/> to this control.
        ...
        protected override void OnMouseDoubleClick(MouseEventArgs e)
        {
            ...
            {
                item = GetTabItemByPoint(e.Location);
                if (item != null)
                {
                    SelectedItem = item;
                }
            }
            if (TabStripItemDBClicked != null)
                TabStripItemDBClicked.Invoke(item, null);
        }
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            {
                if (TabCloseButtonClicked != null) TabCloseButtonClicked.Invoke(null, null);
...
//                 }
        ...
        private void OnCalcTabPage(Graphics g, FATabStripItem currentItem)
        {
            ...
            SizeF textSize = g.MeasureString(currentItem.Title, currentFont, new SizeF(PreferredWidth, 999), sf);
            textSize.Width += 40;
            //textSize.Width = Math.Max(textSize.Width, PreferredWidth);

            if (RightToLeft == RightToLeft.No)
            {
                RectangleF buttonRect = new RectangleF(DEF_START_POS, 3, textSize.Width, Math.Max(17, textSize.Height+6));
                DEF_HEADER_HEIGHT = (int)Math.Max(17, textSize.Height + 6) + 2;
                currentItem.StripRect = buttonRect;
                UpdateLayout();
//                 currentItem.Location = new Point(currentItem.Left, currentItem.Top - 17 + (int)buttonRect.Height);
//                 currentItem.Size = new Size(currentItem.Size.Width, currentItem.Size.Height-17+(int)buttonRect.Height);
        ...
        private void OnDrawTabPage(Graphics g, FATabStripItem currentItem)
        {
            ...
            if (RightToLeft == RightToLeft.No)
            {
                ...
                {
                    path.AddLine(buttonRect.Left+1, buttonRect.Bottom - 1, buttonRect.Left+1,
                                 buttonRect.Bottom - (buttonRect.Height/2) - 2);
                    path.AddLine(buttonRect.Left+1, buttonRect.Bottom - (buttonRect.Height/2) - 3,
                                 buttonRect.Left + (buttonRect.Height/2) - 4+1, mtop + 3);
                }
                ...
                if (currentItem == SelectedItem)
                {
                    brush = new LinearGradientBrush(buttonRect,/* Color.DimGray*/SystemColors.ControlLightLight, SystemColors.Window, LinearGradientMode.Vertical);
                }
                else
                {
                ...
                g.DrawPath(/*SystemPens.ControlDark*/Pens.Black, path);

                if (currentItem == SelectedItem)
                {
                    using (Pen p = new Pen(SystemColors.ControlLightLight, 2))
                    g.DrawLine(p, buttonRect.Left - 9, buttonRect.Height + 2,
        ...
        public void PrevItem()
        {
            ...
            {
                FATabStripItem item = items[i];
                FATabStripItem prev = items[((i-1)+items.Count) % items.Count];
        ...
        public void NextItem()
        {
            ...
            {
                FATabStripItem item = items[i];
                FATabStripItem next = items[(i + 1) % items.Count];
                if (item == SelectedItem)
                {
                    SelectedItem = next;
                    break;
        ...
        public bool IsCurrent(string s)
        {
            if (SelectedItem == null)
                return false;
            ...
            if (!(SelectedItem.Tag is string))
                return false;
            string fullpath = ((DM.Views.LayerView)SelectedItem.Tag).FullPath;
            return fullpath.Equals(s, StringComparison.OrdinalIgnoreCase);
        }
        public FATabStripItem Exist(DM.Views.LayerView c)
        {
            foreach (FATabStripItem item in items)
            {
                if (c.IsEqual(item.Tag as DM.Views.LayerView))
                    return item;
            }
            return null;
        }
        public FATabStripItem Exist(string o)
        {
            foreach (FATabStripItem item in items)
            {
                if (item.Tag == null)
                    continue;
                if (((DM.Views.LayerView)item.Tag).IsEqual(o))
                    return item;
            }
            return null;
        }

        public void CloseWindow(Control sub)
        {
            if (sub == null)
                return;
            if (sub.Parent == null)
                return;
            if (!(sub.Parent is FATabStripItem))
                return;
            FATabStripItem item = (FATabStripItem)sub.Parent;
            TabStripItemClosingEventArgs args = new TabStripItemClosingEventArgs(item);
            OnTabStripItemClosing(args);
            if (!args.Cancel && SelectedItem.CanClose)
            {
                RemoveTab(item);
        ...
        public void CloseCurrent()
        {
            if (SelectedItem == null)
                return;
            CloseWindow((Control)SelectedItem.Tag);
        }
        #endregion
